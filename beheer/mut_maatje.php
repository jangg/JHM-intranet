...
function calculateAge($date)
{
  ...
  $age =
    date(
      "md",
      date("U", mktime(0, 0, 0, $birthDate[1], $birthDate[2], $birthDate[0]))
    ) > date("md")
...
$arr1 = [[0 => "werkzkd.id_maatje", 1 => $mtj->id]];
$arr2 = [[0 => "person.achternaam", 1 => "ASC"]];
...
if (isset($_POST["updateMtBut"]) && $_POST["updateMtBut"] == "wijzig") {
  ...
  $mtj_nw->picfile = $_POST["picfile"];
  if (isset($_POST["mtjcrt_ind"])) {
    $mtj_nw->mtjcrt_ind = $_POST["mtjcrt_ind"];
  } else {
    $mtj_nw->mtjcrt_ind = "n";
  }
  if (isset($_POST["actiefmtj"])) {
    if ($mtj_nw->actief_als == "B" || $mtj_nw->actief_als == "K") {
      $mtj_nw->actief_als = "K";
    } else {
      $mtj_nw->actief_als = "A";
    }
  } else {
    if ($mtj_nw->actief_als == "K") {
      $mtj_nw->actief_als = "B";
    } else {
      if ($mtj_nw->actief_als == "A") {
        $mtj_nw->actief_als = "";
      }
    }
  }

  if (isset($_POST["jglcrt_ind"])) {
    $mtj_nw->jglcrt_ind = $_POST["jglcrt_ind"];
  } else {
    $mtj_nw->jglcrt_ind = "n";
  }
  if (isset($_POST["actiefjgl"])) {
    if ($mtj_nw->actief_als == "A" || $mtj_nw->actief_als == "K") {
      $mtj_nw->actief_als = "K";
    } else {
      $mtj_nw->actief_als = "B";
    }
...
<body style="background-color: #dddddd;">
	...
	<?php
     ...
     ?>
				<img class="card-img-top mb-2" src="../fotoos_person/<?php if ($mtj->picfile == "") {echo "";} else {echo $mtj->picfile;} ?>" style="max-width: 160px;" alt="" />
					<div class="input-group input-group-sm mb-1">
						...
						<select class="form-control" name="picfile" id="picfile">
							...
							<option value="<?php echo $file; ?>" <?php if ($mtj->picfile == $file) {echo " selected";} ?>>
							<?php echo $file; ?>
							</option>
							<?php endif; ?>
							<?php endforeach; ?>
        ...
        echo calculateAge($mtj->date_geboorte);
      } else {
        echo "";
      } ?>" disabled>
					</div>
				...
				<div class="col-md-6 mt-2 pt-2">
					...
					<div class="input-group input-group-sm mb-1">
						...
						<input type="checkbox" name="mtjcrt_ind" id="mtjcrt_ind" class="form-control" value="j" style="margin-left: 15px;" <?php if (
        $mtj->mtjcrt_ind == "j"
      ) {
        echo " checked";
      } ?>>
					...
					<div class="input-group input-group-sm mb-1">
						...
						<input type="checkbox" name="actiefmtj" id="actiefmtj" class="form-control" value="j" style="margin-left: 15px;" <?php if (
        $mtj->actief_als == "A" ||
        $mtj->actief_als == "K"
      ) {
        echo " checked";
					...
					<div class="input-group input-group-sm mb-1">
						...
						<input type="checkbox" name="jglcrt_ind" id="jglcrt_ind" class="form-control" value="j" style="margin-left: 15px;" <?php if (
        $mtj->jglcrt_ind == "j"
      ) {
        echo " checked";
      } ?>>
					...
					<div class="input-group input-group-sm mb-1">
						...
						<input type="checkbox" name="actiefjgl" id="actiefjgl" class="form-control" value="j" style="margin-left: 15px;" <?php if (
        $mtj->actief_als == "B" ||
        $mtj->actief_als == "K"
      ) {
        echo " checked";
