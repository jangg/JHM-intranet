...
function getMaatjeName($id)
{
  ...
  if ($maatje->id == "" || $maatje->id == null) {
    ...
    return $maatje->voornaam .
      " " .
      $maatje->tussenvoegsels .
      " " .
      $maatje->achternaam;
  }
}
...
function getAllPs()
{
  ...
  $sql =
    "SELECT processtap.* FROM processtap WHERE processtap.id_werkzkd = " .
    $_SESSION["werkzkd_id"] .
    ' AND processtap.delind = "n" ORDER BY processtap.dt_stap DESC;';
  $psList = [];
  global $connection;
  try {
    global $nbrPS;
    ...
    error_log(
      "Connectie (processtap 1 in mut_persoon.php) met de database mislukt: " .
        $e->getMessage()
    );
    return false;
  }
  ...
  foreach ($psList as $ps) {
    ...
    $ps_html .=
      '<div class="processtap" style="border: solid 2px #979a97; border-radius: 4px; margin: 3px 0px;">
		...
		<div class="" style="font-size: .8em; display: inline-block; padding: 0px 10px; width: 20%; vertical-align: top;">' .
      Tools::ConvertTS($ps->dt_stap) .
      '</div>
		<div class="" style="font-size: .8em; display: inline-block; padding: 0px 10px; width: 10%; vertical-align: top;">' .
      $user->username .
      '</div>
		<div class="" style="font-size: .8em; display: inline-block; padding: 0px 10px; width: 50%; vertical-align: top;">' .
      $ps->wzstatus .
      " - " .
      (isset($statusArray[$ps->wzstatus])
        ? $statusArray[$ps->wzstatus]
        : "Verandering") .
      '</div>
		<div class="" style="font-size: .8em; display: inline-block; padding: 0px 10px; float: right; vertical-align: top;"><button class="psdel" data-idps=' .
      $ps->id .
      '><i class="fa fa-trash-alt ifont"></i></button></div>
		</div>';
    if ($ps->toelichting != "") {
      $ps_html .=
        '<div class="" style="background-color: #e8eeea; color: #757775;">
				<div class="" style="font-size: .8em; display: inline-block; padding: 0px 10px; white-space: pre-line;">' .
        $ps->toelichting .
        '</div>
			</div>';
    }
...
function getAllAant()
{
  global $nbrAant;
  $sql =
    "SELECT aantekening.* FROM aantekening WHERE aantekening.id_werkzkd = " .
    $_SESSION["werkzkd_id"] .
    ' AND aantekening.delind = "n" ORDER BY aantekening.datetime_created DESC;';
  $aantList = [];
  global $connection;
  try {
    openDB();
    ...
    error_log(
      "Connectie (aantekening 1 in mut_persoon.php) met de database mislukt: " .
        $e->getMessage()
    );
    return false;
  }
  ...
  foreach ($aantList as $aant) {
    ...
    $aant_html .=
      '<div><div class="input-group-text" style="font-size: .8em; display: inline-block;">' .
      Tools::ConvertTS($aant->datetime_created) .
      '</div>
			<div class="input-group-text" style="font-size: .8em; display: inline-block;">' .
      $user->username .
      "</div>";
    $aant_html .=
      '<div class="input-group input-group-sm mb-1">
				...
				<textarea type="textarea" rows="7" class="form-control" disabled>' .
      $aant->tekst .
      '</textarea>
			</div></div>';
  }
...
if (isset($_GET["id"])) {
  ...
  if (!isset($_SESSION["werkzkd_id"])) {
    $_SESSION["werkzkd_id"] = "15";
  }
}
...
if (isset($_POST["deleteWzBut"]) && $_POST["deleteWzBut"] == "delete") {
  ...
  $wkz->delind = "j";
  $wkz->id_user_modified = $curr_user->id;
  $wkz->updateToDB();
  header("location: overz_werkzoekenden.php");
  exit();
}
...
if (isset($_POST["updateWzBut"]) && $_POST["updateWzBut"] == "wijzig") {
  ...
  if ($post_id_maatje != $wkz_nw->id_maatje) {
    ...
    addPS(
      ...
      "999",
      "Maatje veranderd: " .
        getMaatjeName($wkz_nw->id_maatje) .
        " ---> " .
        getMaatjeName($post_id_maatje)
    );
  ...
  if ($post_id_jobgroup != $wkz_nw->id_jobgroup) {
    ...
    addPS(
      ...
      "999",
      "JobGroup veranderd: " .
        getJgName($wkz_nw->id_jobgroup) .
        " ---> " .
        getJgName($post_id_jobgroup)
    );
...
$arr1 = [];
$arr2 = [[0 => "jobgroup.startdate", 1 => "ASC"]];
...
if ($wkz->id_user_modified != "") {
  $user_modified = new User("id", $wkz->id_user_modified);
} else {
			...
			<div class="row mt-4">
				<div class="col-12">
					<div class="d-flex flex-wrap align-items-center gap-2">
						...
						<div class="d-flex flex-wrap align-items-center gap-2">
							<a class="btn btn-primary mr-2" style="min-width:140px" href="intake.php?id=<?php echo (int) $wkz->id; ?>">Intake formulier</a>
							<a class="btn btn-primary mr-2" style="min-width:140px" href="overz_werkzoekenden.php">Terug</a>
						</div>
					</div>
				</div>
			</div>
     ...
     ?>
         ...
         echo $wkz->picfile;
					...
					<div class="input-group input-group-sm mb-1">
						...
						<select class="form-control" name="picfile" id="picfile">
							<option value="">--</option>
							<?php foreach ($files as $file): ?>
								<?php if (is_file($directory . "/" . $file)): ?>
									<option value="<?php echo $file; ?>" 
									<?php if ($wkz->picfile == $file) {
           echo " selected";
         } ?>>
									<?php echo $file; ?>
									</option>
					...
					<div class="input-group input-group-sm mb-1">
						...
						<select class="form-control"  name="geslacht" id="geslacht">
							<option value="" <?php if ($wkz->geslacht == "") {
         echo "selected";
       } ?>>---</option>
							<option value="m" <?php if ($wkz->geslacht == "m") {
         echo "selected";
       } ?>>Man</option>
							<option value="v" <?php if ($wkz->geslacht == "v") {
         echo "selected";
       } ?>>Vrouw</option>
							<option value="n" <?php if ($wkz->geslacht == "n") {
         echo "selected";
       } ?>>Genderneutraal</option>
						</select>
					</div>
					<div class="input-group input-group-sm mb-1">
						<div class="input-group-prepend" style="width: 30%;">
					...
					<div class="input-group input-group-sm mb-1">
						...
						<input type="text" name="datetime_created" class="form-control ps-date" disabled value="<?php echo Tools::ConvertTS(
        $wkz->datetime_created
      ); ?>"/>
					</div>
					<div class="input-group input-group-sm mb-1">
						<div class="input-group-prepend" style="width: 30%;">
						...
						<input type="text" name="datetime_modified" class="form-control" disabled value="<?php echo Tools::ConvertTS(
        $wkz->datetime_modified
      ); ?>"/>
					</div>
					<div class="input-group input-group-sm mb-1">
						<div class="input-group-prepend" style="width: 30%;">
							<span class=" input-group-text text-left text-wrap" style="width: 100%;">Gewijzigd door</span>
						</div>
						<input type="text" name="id_user_modified" class="form-control" disabled value="<?php echo $user_modified->username; ?>"/>
					</div>
					<div class="input-group input-group-sm mb-1">
						<div class="input-group-prepend" style="width: 30%;">
...
// echo $wkz->date_geboorte;
?>" maxlength="10" placeholder="jjjj-mm-dd"> -->
					</div>
					<div class="input-group input-group-sm mb-1">
						<div class="input-group-prepend" style="width: 30%;">
        ...
        echo Tools::calculateAge($wkz->date_geboorte);
      } else {
        echo "";
      } ?>" disabled>
					</div>
				...
				<div class="col-lg-4 mt-2 pt-2" style="background-color:#a5cad8">
					...
					<div class="input-group input-group-sm mb-1">
						...
						<input type="text" name="wzstatus" class="form-control" disabled value="<?php echo $wkz->status .
        " - " .
        (isset($statusArray[$wkz->status])
          ? $statusArray[$wkz->status]
          : "onbekend"); ?>"/>
					</div>
					<div class="input-group input-group-sm mb-1">
						<div class="input-group-prepend" style="width: 30%;">
					...
					<div class="input-group input-group-sm mb-1">
						...
						<select class="form-control"  name="startsituatie" id="startsituatie">
							<option value="" <?php if ($wkz->startsituatie == "") {
         echo "selected";
       } ?>>---</option>
							<option value="nug" <?php if ($wkz->startsituatie == "nug") {
         echo "selected";
       } ?>>Niet uitkering gerechtigd</option>
							<option value="wkw" <?php if ($wkz->startsituatie == "wkw") {
         echo "selected";
       } ?>>WW</option>
							<option value="wrk" <?php if ($wkz->startsituatie == "wrk") {
         echo "selected";
       } ?>>Werk</option>
							<option value="zkw" <?php if ($wkz->startsituatie == "zkw") {
         echo "selected";
       } ?>>Ziektewet</option>
							<option value="bst" <?php if ($wkz->startsituatie == "bst") {
         echo "selected";
       } ?>>Bijstand</option>
							<option value="wza" <?php if ($wkz->startsituatie == "wza") {
         echo "selected";
       } ?>>WIA</option>
							<option value="zzp" <?php if ($wkz->startsituatie == "zzp") {
         echo "selected";
       } ?>>ZZP</option>
						</select>
					</div>
        ...
        echo " checked";
					...
					<div class="input-group input-group-sm mb-1">
						...
						<select class="form-control"  name="opleiding" id="opleiding">
							<option value="" <?php if ($wkz->opleiding == "") {
         echo "selected";
       } ?>>---</option>
         ...
         echo "selected";
							...
							<option value="VWO" <?php if ($wkz->opleiding == "VWO") {
         echo "selected";
       } ?>>VWO</option>
							<option value="MBO1" <?php if ($wkz->opleiding == "MBO1") {
         echo "selected";
       } ?>>MBO 1/2</option>
							<option value="MBO2" <?php if ($wkz->opleiding == "MBO2") {
         echo "selected";
       } ?>>MBO 3/4</option>
							<option value="HBO1" <?php if ($wkz->opleiding == "HBO1") {
         echo "selected";
       } ?>>HBO bachelor</option>
							<option value="HBO2" <?php if ($wkz->opleiding == "HBO2") {
         echo "selected";
       } ?>>HBO master</option>
							<option value="HBO3" <?php if ($wkz->opleiding == "HBO3") {
         echo "selected";
       } ?>>HBO post</option>
							<option value="WO1" <?php if ($wkz->opleiding == "WO1") {
         echo "selected";
       } ?>>WO bachelor</option>
							<option value="WO2" <?php if ($wkz->opleiding == "WO2") {
         echo "selected";
       } ?>>WO master</option>
							<option value="WO3" <?php if ($wkz->opleiding == "WO3") {
         echo "selected";
       } ?>>WO post</option>
						</select>
					</div>
