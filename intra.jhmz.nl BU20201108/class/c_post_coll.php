...
class Post_coll {
    ...
    private function __construct2 ($selectArr, $orderArr) {
	    /*
	    	...
	    	1. Een array met selectiecriteria;
	    		...
	    		Als de array leeg is worden alle posten geselecteerd.
    ...
    private function buildQuery ($selectArr, $orderArr, $limit) {
	    ...
	    $this->query = 'SELECT posts.* FROM posts';
	    if (!empty($selectArr)) {
			$this->query .= ' WHERE ';
...
// 				echo $post;
		...
		} catch (PDOException $e) 
		{
			// error_log($this->query);
			  error_log('Connectie (post 1 in c_post_coll.php) met de database mislukt: ' . $e->getMessage());
			  return FALSE;
		}
