<?php
include_once ('c_topic.php');

class Topic_coll {
	
	protected $topicColl = array();
	protected $query;
    ...
    private function buildQuery ($selectArr, $orderArr, $limit) {
	    ...
	    $this->query = 'SELECT topics.* FROM topics';
	    if (!empty($selectArr)) {
			$this->query .= ' WHERE ';
	...
	private function execQuery () {
		...
		try
		{
			...
			foreach ($rows as $row)
			{
				$topic = new Topic ($row);
// 				echo $topic;
				$this->topicColl[] = $topic;
			}
		} catch (PDOException $e) 
		{
			  echo 'Connectie (topic 1 in c_topic_coll.php) met de database mislukt: ' . $e->getMessage();
			  return FALSE;
		}
