...
class client_coll {
    ...
    private function __construct2 ($selectArr, $orderArr) {
	    /*
	    	...
	    	1. Een array met selectiecriteria;
	    		...
	    		Als de array leeg is worden alle clienten geselecteerd.
    ...
    private function __construct3 ($selectArr, $orderArr, $limit) {
	    /*
	    	...
	    	2. Een array met de volgorde waarin de collection wordt aangeboden
	    		...
	    		attr 2: ASC of DESC
	    		De volgorde si op volgorde van de attributen. Dus sorteren op 1, daarbinnen op 2 enz.
	    	3. het aantal te lezen clienten
    ...
    public function getclientColl () {
    
	    return $this->clientColl;
    }
    
    private function buildQuery ($selectArr, $orderArr, $limit) {
	    
	    $this->query = 'SELECT client.* FROM client';
		$this->query .= ' WHERE client.delind = "n" ';
	    if (!empty($selectArr)) {
...
// 				echo $client;
				$this->clientColl[] = $client;
			}
		} catch (PDOException $e) 
		{
			  error_log('Connectie (client 1 in c_client_coll.php) met de database mislukt: ' . $e->getMessage());
			  return FALSE;
		}
