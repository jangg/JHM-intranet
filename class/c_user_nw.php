...
class User extends Person
{
	...
	protected $password_legacy; // tijdelijk (oude plaintext of md5), voor migratie
	...
	public function __construct1 ($userrow) 
	{
		...
		if ($userrow)
		{
			...
			$this->password_hash  = $userrow['password_hash'] ?? '';
			$this->password_legacy = $userrow['password'] ?? ''; // tijdelijk
	...
	{
		// “nieuwe” manier
		if (!empty($this->password_hash) && password_verify($plain, $this->password_hash)) {
			// rehash als algoritme/params veranderd zijn
		...
		// legacy migratie (tijdelijk): plaintext of md5
		if (!empty($this->password_legacy)) {
			$legacyOk = hash_equals($this->password_legacy, $plain)
				|| hash_equals($this->password_legacy, md5($plain)); // als je oude md5 had
	
			if ($legacyOk) {
				// upgrade naar hash
				$this->password_hash = password_hash($plain, PASSWORD_DEFAULT);
				// (optioneel) legacy leegmaken in DB:
				// $this->password_legacy = '';
				$this->updateToDB();
				return true;
			}
	...
	public function updateToDB () 
	{
		...
		try
		{
			...
			$stmt->bindValue( ":password_legacy" , $this->password, PDO::PARAM_STR);
