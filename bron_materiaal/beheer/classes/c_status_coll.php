<?php
include_once ('c_status.php');

class Status_coll {
	
	protected $statusColl = array();
	protected $query;
    ...
    public function getStatusColl () {
    
	    return $this->statusColl;
    }
    
    private function buildQuery ($selectArr, $orderArr, $limit) {
	    
	    $this->query = 'SELECT status.* FROM status ';
...
//				echo $status;
				$this->statusColl[] = $status;
			}
		} catch (PDOException $e) 
		{
			  echo 'Connectie (status 1 in c_status_coll.php) met de database mislukt: ' . $e->getMessage();
			  return FALSE;
		}
