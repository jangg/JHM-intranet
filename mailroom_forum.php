...
function validateArgs(array $argv): array
{
	...
	if ($argc !== 4 || !is_numeric($argv[1]) || !is_numeric($argv[2]) || !is_numeric($argv[3])) {
		fwrite(STDERR, "Gebruik: php -f mailroom_forum.php <topic_id> <post_id> <user_id>  > /dev/null &\n");
		exit(1);
	}
	return [
		'topic_id' => (int)$argv[1],
...
// onderstaande alleen gebruiken als mail mbv URL wordt evrstuurd.
// deze mailroom_forum.php wordt aangeroepen als nieuwe thread en loopmdus in de background
...
$users = new User_coll([['user.forumNoteInd', 'j']], '');
